...
## Sidekiq部署
因为sidekiq和主app代码完全一样，所以直接在主app的deployment里面复制了一份deployment,改了一下环境变量，然后在app `entrypoint.sh`里面通过环境变量决定是起app还是起sidekiq.
...
kind: Deployment
metadata:
  name: crashreportsidekiq-app
  labels:
    app: crashreportkiq-app
spec:
  strategy:
    type: RollingUpdate
  replicas: 1
